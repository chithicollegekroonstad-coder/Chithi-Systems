...
export async function POST(req: NextRequest) {
  ...
  const {
    ...
    faceDescriptor, // for face verification
    credential, // for fingerprint/WebAuthn
  } = body;
  ...
  if (!locationCheck.ok) {
    ...
    return NextResponse.json(
      { error: "Clock-out not allowed", message: msg },
      { status: 403 },
    );
  }
  ...
  // Find active clock-in record (no clock-out yet)
  ...
  if (!activeRecord || activeRecord.length === 0) {
    return NextResponse.json(
      { error: "No active clock-in found today. Please clock in first." },
      { status: 404 },
    );
  }
  ...
  await db
    ...
    .where(eq(attendanceRecords.id, activeRecord[0].id));
